...
double TempCal = 2;                  //USER SUPPLIED: Cheesy Temp C calibration factor
double baroCal = 5;                  //USER SUPPLIED: Cheesy Barometer calibration factor hPa
...
double humidity = 0;                 //Humidity variable
...
double avgPT[3][avgPTsz+1];          //Store [N+1] press, temp, hum for averages
...
void avgPTCalc(){                    //Calc all avgs
  ...
  avgPT[2][0] = 0;                   //Init Humidity accumulator
  for(int k=1; k<=avgPTsz; k++){     //Loop thru avg readings
    ...
    avgPT[2][0] += avgPT[2][k];      //Add to accumulator for H
  }
  avgPT[0][0] = avgPT[0][0]/avgPTsz; //Put average P at loc 0
  avgPT[1][0] = avgPT[1][0]/avgPTsz; //Put average T at loc 0
  avgPT[2][0] = avgPT[2][0]/avgPTsz; //Put average humidity at loc 0
}
...
//    outBuff = "Temperature: "+String(cTemp)+("°C; ")+String(fTemp)+"°F  ";
    ...
    if (Compare(cTemp,avgPT[1][0],0) == 1){         //Compare(F1,F2,# decimal places compared)
      ...
      if(avgFlag){                                  //Print only if avg valid
        outBuff = "Relative Humidity (avg): "+String(avgPT[2][0])+"%RH";        //Avg humidity
        S_pln(outBuff);print4File(outBuff);}        //Print to COM & WiFi file
    ...
    avgPT[2][avgPTindx] = humidity;                 //Save humidity to avg
